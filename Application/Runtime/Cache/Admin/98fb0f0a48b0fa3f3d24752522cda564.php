...
<form method="post" action="" name="listForm">
    <div class="list-div" id="listDiv">
        <table width="100%" cellspacing="1" cellpadding="2" id="list-table">
            <tr>
                <?php if(is_array($attr)): foreach($attr as $key=>$vo): ?><th><?php echo ($vo["0"]["attr_name"]); ?></th><?php endforeach; endif; ?>
                <th>数量</th>
                <th>操作</th>
            </tr>  
            <?php if(is_array($info)): $keys = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v2): $mod = ($keys % 2 );++$keys;?><tr align="center" class="0">
                <?php if(is_array($attr)): foreach($attr as $key=>$vo): ?><td align="left" class="first-cell" >
                    <select name="attr[<?php echo ($vo["0"]["attr_id"]); ?>][]">
                    <?php if(is_array($vo)): foreach($vo as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php if(in_array(($v["id"]), is_array($v2["goods_attr_ids"])?$v2["goods_attr_ids"]:explode(',',$v2["goods_attr_ids"]))): ?>selected="selected"<?php endif; ?> ><?php echo ($v["attr_values"]); ?></option><?php endforeach; endif; ?>
                    </select>
